...
def create_user(request: schemas.User, db: Session = Depends(get_db)) -> responseModels.ShowUser:
    new_user = models.Users(username=request.username, password=Hash.hash_pswd(request.password), name=request.name, email=request.email, contact_no=request.contact_no)
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
    return new_user
